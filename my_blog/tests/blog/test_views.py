...
class TestAuthorized(ViewTestMixin):
    def test_blog_add_blogpost(self):
        ...
        """
        self.login()
        blogpost = {'title': 'test', 'content': 'testing...', 'published': True,
    ...
    def test_blog_delete_blogpost_get_message(self):
        ...
        """
        self.login()
        response = self.client.get(url_for('blog.delete', post_id=7), follow_redirects=True)
        assert_status_with_message(200, response, 'Please confirm deleting the post.')
    ...
    def test_blog_delete_blogpost(self):
        ...
        """
        self.login()
        response = self.client.post(url_for('blog.delete', post_id=7), follow_redirects=True)
        assert response.status_code == 200

    def test_blog_delete_blogpost_404(self):
        """
        Test that on delete not existing page 404 event handling page is shown
        """
        self.login()
        response = self.client.get(url_for('blog.delete', post_id=1000), follow_redirects=True)
